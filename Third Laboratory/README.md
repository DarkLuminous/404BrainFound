...
## 1. Differentiate Task Parallelism and Data Parallelism.  
Identify which part of the lab demonstrates each and justify the workload division.

### (a) Task Parallelism
Task Parallelism executes different functions concurrently on the same input.
...
### (b) Data Parallelism
Data Parallelism applies the same function to multiple data elements simultaneously.
...
## 2. Explain how `concurrent.futures` managed execution, including the roles of `submit()`, `map()`, and `Future` objects.  
Discuss the purpose of the `with` statement when creating an Executor.
...
## 3. Analyze the execution of `ThreadPoolExecutor` in relation to the Global Interpreter Lock (GIL) and CPU cores.  
Did true parallelism occur? Explain your answer.

In `task.py`, `ThreadPoolExecutor` uses threads within a single process.  
...
Discuss memory space separation and GIL behavior.

In `data.py`, `ProcessPoolExecutor` creates separate processes. Each process:
...
## 5. Evaluate the scalability of the system if the number of employees increases from 5 to 10,000.  
Which approach scales better and why?

If employees increase to 10,000:

- `task.py` (Task Parallelism) would not scale efficiently because it only parallelizes deductions within one employee at a time.  
- `data.py` (Data Parallelism) scales better because each employee’s payroll is independent and can be distributed across multiple processes and CPU cores.  

Therefore, the data-parallel approach in `data.py` is more scalable.

---

## 6. Provide a real-world payroll system example.  
Indicate where Task Parallelism and Data Parallelism would be applied, and which executor you would use for each case.
